...
// Données formulaire (classes + matières + enseignants) — admin
const getFormData = asyncHandler(async (req, res) => {
  const [classes]  = await db.execute('SELECT id, nom, niveau FROM classes ORDER BY nom')
  const [matieres] = await db.execute('SELECT id, nom FROM matieres ORDER BY nom')
...
// — route /mine  : teacher_id = req.user.id
// — route /teacher/:id : teacher_id = req.params.teacher_id
...
const create = asyncHandler(async (req, res) => {
  ...
  if (heure_debut >= heure_fin)
    return res.status(400).json({
      success: false, message: "L'heure de fin doit être après l'heure de début.",
    })
...
module.exports = { getFormData, getByClasse, getByTeacher, create, update, remove }